...
### Hotfix: Polars Optional Dependency
- **fix(compat):** Resolved a critical `ModuleNotFoundError` crash. The engine now gracefully guards internal imports with `HAS_POLARS` and `HAS_PANDAS` flags, strictly preserving stability for Pandas-only environments where the optional Polars backend is not installed.
